...
if (!allMode) {
  ...
  for (const { status, file } of changed) {
    ...
    if (!file.endsWith(".md")) fail(`Only Markdown files may change, found ${file}`);
    if (status.startsWith("D")) fail(`Deleting content is not allowed in auto-merge PRs: ${file}`);
  }
}

const seen = new Map();
for (const file of run(["ls-files", "content"]).split("\n").filter(Boolean)) {
...
console.log(allMode ? "Content validation passed for all Markdown files." : `Content PR validation passed for ${changed.length} changed file(s).`);
