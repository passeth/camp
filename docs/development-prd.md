...
Camp is an AI-powered study magazine, wiki, and archive platform. MVP ships as a Next.js + Supabase + Git-backed Markdown web app. Published content is read from Markdown files. Supabase stores auth, member roles, publish requests, comments, reactions, views, and metadata. Hermes Agent, VPS, Obsidian sync, and automated Git publish jobs are future integration points.
...
- Content source of truth: Markdown files under `content/`.
- Web editor behavior: creates Supabase `publish_requests`; approval does not yet mutate Git.
...
- Approved members can submit publish requests from `/write`.
